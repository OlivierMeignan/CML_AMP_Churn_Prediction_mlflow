...
subprocess.run(
        "pip install git+https://github.com/fastforwardlabs/cmlbootstrap#egg=cmlbootstrap --upgrade", shell=True
    )
...
# Set the HIVE_TABLE environment variable
try:
    hive_table = os.environ["HIVE_TABLE"]
except:
    rc = cml.create_environment_variable({"HIVE_TABLE": hive_table})
    os.environ["HIVE_TABLE"] = hive_table  



# Set the HIVE_DATABASE environment variable
try:
    hive_database = os.environ["HIVE_DATABASE"]
except:
    rc = cml.create_environment_variable({"HIVE_DATABASE": hive_database})
    os.environ["HIVE_DATABASE"] = hive_database   


# Set the DATA_LOCATION environment variable
try:
    data_location = os.environ["DATA_LOCATION"]
except:
    rc = cml.create_environment_variable({"DATA_LOCATION": data_location})
    os.environ["DATA_LOCATION"] = data_location   
...
try:
    ...
    rc = cml.create_environment_variable({"STORAGE": storage})
